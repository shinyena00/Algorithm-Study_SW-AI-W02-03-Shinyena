...
def find_above_average_students(students):
    ...
    student = [student["score"] for student in students]
    
    # TODO: 평균 점수를 계산하세요
    average = sum(student) / len(students)
    
    # TODO: 평균 이상인 학생들의 이름을 리스트로 추출하세요
    above_average_students = [result["name"] for result in students if result["score"] >= average]
